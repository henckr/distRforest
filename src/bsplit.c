...
bsplit(pNode me, int n1, int n2)
{
    ...
    /*
     * Create a vector to hold the candidate splits
     * Use the sample function to get the candidates for this split 
     */
    int candidates[rp.ncand];
    sample(candidates, rp.nvar, rp.ncand);

    /*
     * test out the variables 1 at at time
     * 
     * EDIT: only iterate over the current candidates instead of all the variables
     * for loop: for (i = 0; i < rp.nvar; i++) --> for (i = 0; i < rp.ncand; i++)
     * inside the loop: i --> candidates[i]
     */
    me->primary = (pSplit) NULL;
    for (i = 0; i < rp.ncand; i++) {
      index = rp.sorts[candidates[i]];
      nc = rp.numcat[candidates[i]];
      /* extract x and y data */
      k = 0;
      for (j = n1; j < n2; j++) {
        kk = index[j];
        if (kk >= 0 && rp.wt[kk] > 0) {  /* x data not missing and wt > 0 */
      xtemp[k] = rp.xdata[candidates[i]][kk];
          ytemp[k] = rp.ydata[kk];
          wtemp[k] = rp.wt[kk];
          k++;
        }
      ...
      */
      ...
      if (improve > (rp.iscale * 1e-10)) {
        improve /= rp.vcost[candidates[i]];     /* scale the improvement */
      tsplit = insert_split(&(me->primary), nc, improve, rp.maxpri);
      if (tsplit) {
        tsplit->improve = improve;
        tsplit->var_num = candidates[i];
        tsplit->spoint = split;
        tsplit->count = k;
        if (nc == 0) {
          tsplit->spoint = split;
