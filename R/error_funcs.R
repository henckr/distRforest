...
#' @keywords internal
error_funcs <- function(ytrue, ypred, wcase = 1, method) {
...
#' @keywords internal
dev_gamma <- function(ytrue, ypred, wcase = 1) -2 * mean(wcase * (log(ytrue/ypred) - ((ytrue - ypred) / ypred)), na.rm = TRUE)


#' @describeIn error_funcs Weighted mean squared error loss function for anova and lognormal regression
#' @keywords internal
wgt_mse <- function(ytrue, ypred, wcase = 1) mean(wcase * (ytrue - ypred)^2, na.rm = TRUE)


#' @describeIn error_funcs Matthews correlation coefficient for binary classification
#' @keywords internal
mcc <- function(ytrue, ypred, wcase = 1) {
