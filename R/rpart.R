...
#	numy <- init$numy
	...
	method.int <- pmatch(method, c("anova", "poisson", "class", "exp", "lognormal", "gamma"))
	if (is.na(method.int)) stop("Invalid method")
	method <- c("anova", "poisson", "class", "exp", "lognormal", "gamma")[method.int]
	if (method.int == 4L) method.int <- 2L
	
	# Generate warning message for unimplemented lognormal and gamma cross-validation
    ...
    xval <- controls$xval
    if (method %in% c('lognormal','gamma') && xval > 0L) warning('Internal cross-validation through xval>0 in rpart.control is not implemented for method=lognormal and method=gamma.')
    ...
    # Use all the variables as candidates for splitting when ncand is not specified
    form.char <- deparse(formula, width.cutoff = 500)
    if (missing(ncand)) ncand <- lengths(regmatches(form.char, gregexpr("\\+", form.char))) + 1
    
    # Set default for the seed
